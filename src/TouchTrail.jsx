...
class Bloom {
  ...
  draw(ctx, now) {
    ...
    if (!this.lifted) {
      ...
      // Smooth position follow when dragging
      if (this._targetX !== undefined) {
        this.x += (this._targetX - this.x) * 0.18;
        this.y += (this._targetY - this.y) * 0.18;
      }
...
export default function TouchTrail() {
  ...
  useEffect(() => {
    ...
    // Helper — is this touch point on or near an interactive element?
    const BUTTON_PAD = 22; // px padding around buttons to suppress bloom
    ...
    // touchstart → create bloom (skip if on/near interactive element)
    function onStart(e) {
      const W = window.innerWidth;
      Array.from(e.changedTouches).forEach(touch => {
        if (nearInteractive(touch.clientX, touch.clientY)) return;
        blooms.current.set(
          touch.identifier,
    ...
    // touchmove → spawn trailing blooms along the drag path
    function onMove(e) {
      const W = window.innerWidth;
      Array.from(e.changedTouches).forEach(touch => {
        if (nearInteractive(touch.clientX, touch.clientY)) return;
        // Lift any existing bloom for this finger and start a fresh small one
        const existing = blooms.current.get(touch.identifier);
        if (existing && !existing.lifted) {
          // Smooth follow — bloom trails the finger
          existing._targetX = touch.clientX;
          existing._targetY = touch.clientY;
        } else {
          // No active bloom — start one (drag entered from edge)
          const b = new Bloom(touch.clientX, touch.clientY, W);
          blooms.current.set(touch.identifier, b);
        }
      });
    }
